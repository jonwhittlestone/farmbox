...
class TestDropboxFileOperations:
    ...
    def test_remote_folders_can_be_created_on_init(self):
        ...
        assert sorted(subfolders) == sorted(list(settings.CLOUD_SUBFOLDERS))
    ...
    def test_orders(self):
        ...
        t.upload(settings.SAMPLE_ORDER_SHEET_PATH, settings.TESTING_ORDERS_REMOTE_PATH)
    ...
    def test_downloaded_orders_match_file_metadata(self, test_orders):
        ...
        self.clean_up()
        extracted_dir = os.path.join(settings.MEDIA_ROOT, settings.TESTING_DIRNAME)
